...
{
    public const ID = 'modern';

    private const ACCENT = '#f5b301';
    ...
    public static function build(array $ctx): array
    {
        ...
        $navGallery = count($ctx['photos']) ? '<a href="#gallery">Work</a>' : '';
        ...
        $heroRating = $ctx['rating'] ? "<p class=\"mk-hero-rating\">{$star}<strong>{$ctx['rating']}</strong><span>{$ctx['reviewCount']} review{$reviewWord}</span></p>" : '';
        $stripHours = $ctx['workingTime'] ? "<div class=\"mk-strip-item\">{$clock}<div><strong>Open</strong><span>{$ctx['workingTime']}</span></div></div>" : '';
        ...
        $socialSection = $ctx['instagram'] ? <<<HTML
            ...
            HTML : '';
        $mapSection = Helpers::mapSection($ctx, 'Get Directions');
        ...
        $enquiryForm = Helpers::enquiryForm('Send Message');
        ...
        <header class="mk-header">
          <div class="mk-header-inner">
            ...
            <a class="mk-header-cta" href="tel:{$ctx['phone']}">{$phone}<span>{$ctx['phone']}</span></a>
          </div>
        </header>
        ...
        <section class="mk-strip">
          <div class="mk-strip-item">{$pin}<div><strong>Find us</strong><span>{$ctx['address']}</span></div></div>
          {$stripHours}
          <div class="mk-strip-item">{$phone}<div><strong>Talk to us</strong><span><a href="tel:{$ctx['phone']}">{$ctx['phone']}</a></span></div></div>
        </section>
        ...
        @media(prefers-reduced-motion:reduce){.mk-track{animation:none}.mk-tile img,.mk-btn{transition:none}}
        CSS;
