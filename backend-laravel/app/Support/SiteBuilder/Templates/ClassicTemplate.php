...
class ClassicTemplate
{
    ...
    {
        $slides = Helpers::slideshow($ctx);
        $floats = Helpers::floatButtons($ctx, '#e11d2e');
        $phone = Icons::PHONE;
        ...
        $heroEyebrow = $ctx['category'] ? '<span class="mk-eyebrow">'.$ctx['category'].($ctx['city'] ? " in {$ctx['city']}" : '').'</span>' : '';
        $heroHours = $ctx['workingTime'] ? "<p class=\"mk-hero-hours\">{$clock}<span>Open {$ctx['workingTime']}</span></p>" : '';
        $workingFact = $ctx['workingTime'] ? "<div class=\"mk-fact\">{$clock}<div><strong>Working hours</strong><span>{$ctx['workingTime']}</span></div></div>" : '';
        ...
        $socialSection = $ctx['instagram'] ? <<<HTML
            <section class="mk-social">
              <a href="https://www.instagram.com/{$ctx['instagram']}/" target="_blank" rel="noreferrer">{$instagram}<span>Follow us on Instagram</span></a>
            </section>
            HTML : '';
        $mapSection = Helpers::mapSection($ctx);
        ...
        $enquiryForm = Helpers::enquiryForm();
        ...
        <header class="mk-header">
          <div class="mk-header-inner">
            ...
            <a class="mk-header-cta" href="tel:{$ctx['phone']}">{$phone}<span>Call Now</span></a>
          </div>
        </header>
          ...
          <div class="mk-hero-overlay">
            <div class="mk-hero-inner">
              ...
              <div class="mk-hero-actions">
                <a class="mk-btn mk-btn-primary" href="tel:{$ctx['phone']}">{$phone}<span>Call Now</span></a>
                <a class="mk-btn mk-btn-wa" href="{$ctx['whatsappUrl']}" target="_blank" rel="noreferrer">{$whatsapp}<span>WhatsApp</span></a>
        ...
        <section class="mk-section" id="about">
          ...
          <div class="mk-facts">
            <div class="mk-fact">{$pin}<div><strong>Visit us</strong><span>{$ctx['address']}</span></div></div>
            {$workingFact}
            <div class="mk-fact">{$phone}<div><strong>Call us</strong><span><a href="tel:{$ctx['phone']}">{$ctx['phone']}</a></span></div></div>
          </div>
        </section>
        ...
        @media(prefers-reduced-motion:reduce){.mk-track{animation:none}.mk-card,.mk-card-img img,.mk-btn{transition:none}}
        CSS;
