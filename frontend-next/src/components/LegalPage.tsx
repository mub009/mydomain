...
export function Ph({ children }: { children: React.ReactNode }) {
  return <mark className="rounded bg-amber-100 px-1 py-0.5 font-mono text-[13px] font-semibold text-amber-800">{children}</mark>;
}
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <div className="card mt-4 p-5">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1.5">
          {LEGAL_LINKS.map((l) => (
            <Link key={l.href} href={l.href} className="text-sm font-medium text-brand-600 hover:underline">
              {l.label}
            </Link>
          ))}
        </div>
