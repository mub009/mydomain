 ...
 */
export const vibrantTemplate: SiteTemplate = {
  id: "vibrant",
  name: "Vibrant",
...
<header class="mk-header">
  <div class="mk-header-inner">
    ...
    <a class="mk-header-cta" href="${ctx.whatsappUrl}" target="_blank" rel="noreferrer">${ICON.whatsapp}<span>WhatsApp</span></a>
  </div>
</header>
  ...
  <div class="mk-top-inner">
    <div class="mk-top-copy">
      ...
      <div class="mk-hero-actions">
        <a class="mk-btn mk-btn-primary" href="tel:${ctx.phone}">${ICON.phone}<span>Call ${ctx.phone}</span></a>
        <a class="mk-btn mk-btn-ghost" href="${ctx.directionsUrl}" target="_blank" rel="noreferrer">${ICON.pin}<span>Directions</span></a>
      </div>
      ${
        ctx.rating
          ? `<p class="mk-rating">${ICON.star}<strong>${ctx.rating}</strong><span>from ${ctx.reviewCount} review${ctx.reviewCount === 1 ? "" : "s"}</span></p>`
          : ""
      }
    </div>
    <div class="mk-hero">
...
<section class="mk-chips">
  <div class="mk-chip">${ICON.pin}<span>${ctx.address}</span></div>
  ${ctx.workingTime ? `<div class="mk-chip">${ICON.clock}<span>${ctx.workingTime}</span></div>` : ""}
  ${ctx.email ? `<div class="mk-chip">${ICON.mail}<span><a href="mailto:${ctx.email}">${ctx.email}</a></span></div>` : ""}
</section>
...
@media(prefers-reduced-motion:reduce){.mk-track{animation:none}.mk-bubble,.mk-btn{transition:none}}`.trim();
